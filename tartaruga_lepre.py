...
Per la tartaruga eseguite un "passo veloce" quando 1 ≤ i ≤ 5, una "scivolata" quando 6 ≤ i ≤ 7, 
...
i contendenti si troveranno sullo stesso quadrato. In questo caso la tartaruga morde la lepre e il vostro programma 
...
ùImplementare un sistema dove le condizioni 'soleggiato' e 'pioggia' cambiano dinamicamente ogni 10 tick dell'orologio.
...
3. Ostacoli e Bonus
Sulla pista di gara sono presenti alcuni ostacoli e bonus a posizioni fisse, che influenzano direttamente il movimento degli 
animali quando vengono calpestati. Gli ostacoli causano uno slittamento all'indietro, mentre i bonus offrono un avanzamento extra.
...
di un numero specificato di quadrati (es: -3, -5, -7). 
Gli ostacoli sono rappresentati da un dizionario che mappa le posizioni degli ostacoli sul percorso (chiave) ed i relativi effetti 
(valore). Assicurarsi che nessun animale retroceda al di sotto del primo quadrato a seguito di un ostacolo.

- Bonus:
Dislocati strategicamente lungo la corsa (es. ai quadrati 10, 25, 50), i bonus aumentano la posizione dell'animale 
di un numero determinato di quadrati (es: 5, 3, 10). I bonus sono rappresentati da un dizionario che mappa le posizioni 
...
def posizioni_sul_percorso():
    percorso_grafico=[]
    for i in range(1,71):
        if i==posizione_lepre and posizione_lepre==posizione_tartaruga:
            i="T OUCH!!! H"
            percorso_grafico.append(i)
        elif i==posizione_tartaruga:
            i="T"
            percorso_grafico.append(i)
        elif i==posizione_lepre:
            i="H"
            percorso_grafico.append(i)
        else:
            i="_"
...
def mossa_tartaruga():
    ...
    num_casual_tartaruga=random.randint(1,10)
    if num_casual_tartaruga <= 5:
        posizione_tartaruga+=3
    elif num_casual_tartaruga>5 and num_casual_tartaruga<8:
        if posizione_tartaruga>6:
            posizione_tartaruga-=6
        else:
            posizione_tartaruga=1
    elif num_casual_tartaruga>=8:
        posizione_tartaruga+=1


def mossa_lepre():
    global posizione_lepre
    num_casual_lepre=random.randint(1,10)
    if num_casual_lepre>2 and num_casual_lepre<5:
        posizione_lepre+=9
    elif num_casual_lepre==5:
        if posizione_lepre>12:
            posizione_lepre-=12
        else:
            posizione_lepre=1
    elif num_casual_lepre>5 and num_casual_lepre<9:
        posizione_lepre+=1
    elif num_casual_lepre>8:
...
while posizione_lepre<70 or posizione_tartaruga < 70:
    ...
    if posizione_lepre>=70 and posizione_tartaruga>=70:
        print("ITS A TIE")
        break
